...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ chatId: string }> }) {
  try {
    ...
    const data = await gate.ctx.api.markChatAsUnread(gate.ctx.accountId, resolved.chatId)
    return NextResponse.json(data)
  } catch (error) {
    console.error('[fansly/chats/unread] POST', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Failed to mark chat unread' },
      { status: 500 },
    )
  }
}
