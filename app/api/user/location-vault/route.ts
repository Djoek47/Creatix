...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!fallback) {
      // Graceful default when profile columns are unavailable and no fallback exists yet.
      return NextResponse.json({
    ...
    console.error('[user/location-vault][GET]', error)
    return NextResponse.json({ error: 'Failed to load location settings' }, { status: 500 })
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      if (authError) {
        console.error('[user/location-vault][PATCH] profile+auth fallback failed', {
    ...
    console.error('[user/location-vault][PATCH]', error)
    return NextResponse.json({ error: 'Failed to save location' }, { status: 500 })
  }
}
