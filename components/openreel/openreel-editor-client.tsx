...
function CreatixBridge() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    const bridgeKey = `creatix-editor:import:${activeBridge.importUrl}:${activeBridge.exportUrl}`
    ...
    async function importVaultAsset() {
      ...
      try {
        ...
        toast.info('Creatix vault connected', 'Importing your selected video into the editor.')
  ...
  const saveToCreatix = async () => {
    if (!bridge.exportUrl || !bridge.exportToken) {
      toast.error('Vault export unavailable', 'Open the editor from a Creatix vault item to save back.')
      return
    }
    ...
    try {
      ...
      const prepare = await fetch(exportEndpoint(bridge.exportUrl, 'prepare'), {
        method: 'POST',
        credentials: 'include',
      ...
      if (prepare.ok) {
        const prep = (await prepare.json()) as { signedUrl?: string; path?: string; error?: string }
        if (!prep.signedUrl || !prep.path) throw new Error(prep.error || 'Could not prepare vault upload')

        setMessage('Uploading to Creatix vault...')
        const upload = await fetch(prep.signedUrl, {
          method: 'PUT',
          headers: { 'Content-Type': blob.type || 'video/mp4' },
          body: blob,
        })
        if (!upload.ok) throw new Error(`Vault upload failed (${upload.status})`)

        response = await fetch(exportEndpoint(bridge.exportUrl, 'complete'), {
          method: 'POST',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        response = await fetch(bridge.exportUrl, {
          method: 'POST',
          credentials: 'include',
          body: formData,
        })
