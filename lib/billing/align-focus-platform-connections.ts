...
export type FocusAlignPreloaded = {
  ...
  platformRows: { platform: string; is_connected: boolean | null; access_token?: string | null; platform_username?: string | null }[]
}
...
export async function maybeAlignFocusPlatformConnections(
  ...
  type ConnRow = (typeof platformRows)[number]
  const rowByPlatform = new Map<string, ConnRow>()
  ...
  for (const platform of toDisconnect) {
    ...
    if (platform === 'onlyfans') {
      ...
      const token = row.access_token != null && String(row.access_token).trim() !== '' ? String(row.access_token) : null
      if (apiKey && token) {
        try {
          const api = createOnlyFansAPI()
          const deleteResult = await api.deleteAccount(token)
          if (!deleteResult.success) {
            console.warn('[align-focus-platforms] OnlyFans partner delete failed:', deleteResult.message)
          }
        } catch (e) {
          console.warn('[align-focus-platforms] OnlyFans partner delete error:', e)
        }
      }
      ...
      if (error) {
        console.warn('[align-focus-platforms] OnlyFans DB update failed:', error.message)
        continue
      }
      dbDisconnected = true
      ...
      const { error } = await supabase
        ...
        .eq('platform', 'fansly')
      if (error) {
        console.warn('[align-focus-platforms] Fansly DB update failed:', error.message)
        continue
      }
      dbDisconnected = true
    }
  ...
  const removedNames = disconnectedOk.map(platformLabel).join(', ')
